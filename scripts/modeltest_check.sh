...
if [[ "$DOWNLOAD_DATASET_IF" == "ON" ]]; then
    echo '[code-set-convert] Start downloading datasets.....'
...
echo '[code-set-convert] Start converting code set'
failed_projects=()
...
python tools/modeltest/modeltest_check.py;check_error2=$?
...
if [ ${check_error1} != 0  ]; then
    echo "[code-set-convert] Your PR code-test-set (more than 20W+ lines) convert check failed."
    echo "[code-set-convert] The following projects failed to convert:"
    printf '%s\n' "${failed_projects[@]}"
else
    echo "[code-set-convert] Your PR code-test-set (more than 20W+ lines) convert check passed."
fi

if [ ${check_error2} != 0  ]; then
    echo "[modeltest] Your PR code model test check failed."
else
    echo "[modeltest] Your PR code model test check passed."
fi
echo '************************************************************************************************************'
