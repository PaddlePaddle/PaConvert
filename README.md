...
这是由于两者API的用法差异，无法通过一行完成，必须增加若干辅助行来实现相同功能。
...
paddleconverter -in_dir torch_project --out_dir paddle_project [--exclude_dirs exclude_dirs] [--log_dir log_dir] [--log_level level] [--run_check 1]
```
...
转换结束后，会在终端打印出 **转换总结** ，其中包含 **未转换的API、文件及行数**，同时会在当前目录下的 `convert.log` 打印一份相同内容的日志。

例如，上述该文件里一共有9个Pytorch API，其中8个被成功转换，因此转换率为88.89%，如果项目中有多个python文件，则会统计所有文件的累计数据。

对于转换成功的API，代码风格上略有变化，会 **补全API全名、参数关键字、移除注释、移除多余空行** 。因为语法树重新转换为源码时，会采用标准写法来生成代码，而注释、空行等代码无法被语法树识别，将在 `源码->语法树->源码` 的过程中被移除。

对于转换失败的API，将 **补全为torch API全名**，同时在行前通过 `>>>` 的形式加以标记，用户必须对该torch API进行手动转换，可参考[Pytorch-Paddle API映射表](https://www.paddlepaddle.org.cn/documentation/docs/zh/guides/model_convert/pytorch_api_mapping_cn.html#pytorch-1-8-paddle-2-0-api) 进行手动转换，然后删除标记，否则代码无法运行。
...
- 一致的API：要求API功能一致，且API参数一致（如果Pytorch只比Paddle多out/dtype/device/layout/requires_grad/memory_format/inplace/generator/pin_memory参数，则也视作一致），直接增加json配置即可，最为容易

- 不一致但可转换的API：包含 **Pytorch参数更多、参数不一致、API功能不一致、组合实现** 这几种情况，需要开发AST转换策略

- 不一致且无法转换的API：无法转换，可提供映射关系，方便手动转换，见[Pytorch-Paddle API映射表](https://www.paddlepaddle.org.cn/documentation/docs/zh/guides/model_convert/pytorch_api_mapping_cn.html#pytorch-1-8-paddle-2-0-api) 
