...
对任意case只允许有两种情况：a)正常转换且结果一致；b)不支持转换，此时返回None即可。不允许出现其他的错误情况，包括不限于 报错推出、错误转换 等问题。
...
**b) 将所有可能的用法case后，将其全部加入到单测中，并对比结果一致**。要求至少编写5种case（越多约好）。单测写法为：（待补充）

**c) 需要使用验证集通过**。等待流水线xxx通过即可。

**d) 维护与负责**。由于考仍可能考虑的不够全面，可能会引入非常隐蔽的用法bug，开发者需要对自身开发的API转换规则，负责并后续维护。解决新发现的case问题。
