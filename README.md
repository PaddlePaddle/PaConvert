...
**b) 需考虑所有可能的torch用法case**，另外从验证集中抽取尽可能多的用法case，要求至少列举5种完全不同的case（越多约好）。涉及到多个参数的，应包含各种参数组合的情况，不能只考虑最简单最常见的用法。
...
**c) 写出所有可能的torch用法case后，全部加入到单测中，并对比结果一致**。单测写法为：（待补充）
