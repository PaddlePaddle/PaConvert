...
class BasicTransformer(BaseTransformer):
    ...
    def trans_class_attribute(self, node, torch_api):
        ...
        annotate_node = ast.parse(
            "'Not Support auto convert {}, please check whether it is torch.Tensor.* and convert by yourself'".format(
                torch_api
            )
        ).body[0]
    ...
    def trans_class_method(self, node, torch_api):
        ...
        annotate_node = ast.parse(
            "'Not Support auto convert {}, please check whether it is torch.Tensor.*/Module.*/Optimizer.* and convert by yourself'".format(
                torch_api
            )
        ).body[0]
        self.record_scope(self.scope_body_index(), annotate_node)
