...
class NarrowCopyMatcher(BaseMatcher):
    def generate_code(self, kwargs):
        ...
        start = get_unique_name('start')
        code = API_TEMPLATE.format(start, kwargs['input'], kwargs['dim'], kwargs['start'], kwargs['start'], kwargs['start'], 
                kwargs['input'], kwargs['dim'], start, start, kwargs['length'], kwargs['out'])
        return code
...
class HStackMatcher(BaseMatcher):
    def generate_code(self, kwargs): 
        if 'out' in kwargs and kwargs['out'] is not None:          
            ...
            code = API_TEMPLATE.format(axis, kwargs['tensors'], kwargs['tensors'], axis, kwargs['out'])
        else:
            API_TEMPLATE = textwrap.dedent(
                '''  
                {} = 0 if {}[0].ndim == 1 else 1
...
class FModMatcher(BaseMatcher):
    def generate_code(self, kwargs): 
        if 'out' in kwargs and kwargs['out'] is not None:          
            ...
            code = API_TEMPLATE.format(kwargs['input'], kwargs['other'], kwargs['input'], kwargs['out'])
        else:
            API_TEMPLATE = textwrap.dedent(
                '''  
            ...
            code = API_TEMPLATE.format(kwargs['input'], kwargs['other'], kwargs['input'])
...
class LdExpMatcher(BaseMatcher):
    def generate_code(self, kwargs): 
        if 'out' in kwargs and kwargs['out'] is not None:          
            API_TEMPLATE = textwrap.dedent(
                '''  
                paddle.assign(paddle.multiply({}, paddle.pow(paddle.to_tensor(2, dtype={}.dtype), {})), output={})
                '''
            )
            code = API_TEMPLATE.format(kwargs['input'], kwargs['other'], kwargs['other'], kwargs['out'])
        else:
            API_TEMPLATE = textwrap.dedent(
                '''  
                paddle.multiply({}, paddle.pow(paddle.to_tensor(2, dtype={}.dtype), {}))
                '''
            )
...
class StdMeanMatcher(BaseMatcher):
    def generate_code(self, kwargs):
        ...
        if 'out' in kwargs and kwargs['out'] is not None:          
            ...
            code = API_TEMPLATE.format(kwargs['input'], kwargs['dim'], kwargs['correction'], kwargs['keepdim'], kwargs['input'], kwargs['dim'], kwargs['keepdim'], kwargs['out'])
        else:
            API_TEMPLATE = textwrap.dedent(
                '''  
...
class AddMRMatcher(BaseMatcher):
    def generate_code(self, kwargs):
        ...
        if 'out' in kwargs and kwargs['out'] is not None:          
            ...
            code = API_TEMPLATE.format(kwargs['beta'], kwargs['input'], kwargs['alpha'], self.get_paddle_api(), param1, param2, kwargs['out'])
        else:
            API_TEMPLATE = textwrap.dedent(
                '''  
                paddle.add({}*{}, {}*{}({}, {}))
            ...
            code = API_TEMPLATE.format(kwargs['beta'], kwargs['input'], kwargs['alpha'], self.get_paddle_api(), param1, param2)
...
class AddBmmMatcher(BaseMatcher):
    def generate_code(self, kwargs):
        ...
        if 'out' in kwargs and kwargs['out'] is not None:          
            ...
            code = API_TEMPLATE.format(kwargs['beta'], kwargs['input'], kwargs['alpha'], kwargs['batch1'], kwargs['batch2'], kwargs['out'])
        else:
            API_TEMPLATE = textwrap.dedent(
                '''  
...
class CholeskyInverseMatcher(BaseMatcher):
    def generate_code(self, kwargs):
        ...
        if 'out' in kwargs and kwargs['out'] is not None:          
            ...
            perm = get_unique_name('perm')
            out = get_unique_name('out')
            code = API_TEMPLATE.format(perm, kwargs['input'], perm, perm, perm, perm, kwargs['upper'], out, kwargs['input'], perm, 
                    kwargs['input'], out, kwargs['input'], kwargs['input'], perm, out, kwargs['out'])
        else:
            API_TEMPLATE = textwrap.dedent(
                '''  
                {} = list(range({}.ndim))
            ...
            out = get_unique_name('out')
            code = API_TEMPLATE.format(perm, kwargs['input'], perm, perm, perm, perm, kwargs['upper'], out, kwargs['input'], perm, 
                    kwargs['input'], out, kwargs['input'], kwargs['input'], perm, out)
...
class TensorBernoulli_Matcher(BaseMatcher):
    def get_paddle_class_nodes(self, func, args, kwargs):
        ...
        code = API_TEMPLATE.format(bern, kwargs['p'], kwargs['p'], kwargs['p'], bern, self.paddleClass, self.paddleClass)
...
class TensorNarrowMatcher(BaseMatcher):
    def get_paddle_class_nodes(self, func, args, kwargs):
        ...
        start = get_unique_name('start')
        code = API_TEMPLATE.format(start, self.paddleClass, kwargs['dim'], kwargs['start'], kwargs['start'], kwargs['start'], 
                self.paddleClass, kwargs['dim'], start, start, kwargs['length'])
...
class TensorSwapAxesMatcher(BaseMatcher):
    def get_paddle_class_nodes(self, func, args, kwargs):
        ...
        perm = get_unique_name('perm')
        code = API_TEMPLATE.format(perm, self.paddleClass, 
                perm, kwargs['axis0'], kwargs['axis1'], 
                perm, kwargs['axis1'], kwargs['axis0'], 
...
class UnflattenMatcher(BaseMatcher):
    def generate_code(self, kwargs):
        ...
        code = API_TEMPLATE.format(kwargs['input'], kwargs['input'], kwargs['dim'], kwargs['sizes'], kwargs['input'], kwargs['dim'], kwargs['dim'])
...
class TensorUnflattenMatcher(BaseMatcher):
    def get_paddle_class_nodes(self, func, args, kwargs):
        ...
        code = API_TEMPLATE.format(self.paddleClass, self.paddleClass, kwargs['dim'], kwargs['sizes'], self.paddleClass, kwargs['dim'], kwargs['dim'])
...
class IndexAddMatcher(BaseMatcher):
    def generate_code(self, kwargs):
        ...
        if 'out' in kwargs and kwargs['out'] is not None:
            ...
            code = API_TEMPLATE.format(kwargs['input'], kwargs['index'], kwargs['dim'], kwargs['alpha'], kwargs['source'], kwargs['out'])
            ...
            code = API_TEMPLATE.format(kwargs['input'], kwargs['index'], kwargs['dim'], kwargs['alpha'], kwargs['source'])
...
class TensorIndexAddMatcher(BaseMatcher):
    def get_paddle_class_nodes(self, func, args, kwargs):
        ...
        code = API_TEMPLATE.format(self.get_paddle_api(), self.paddleClass, kwargs['index'], kwargs['dim'], kwargs['alpha'], kwargs['source'])
...
class AMinMaxMatcher(BaseMatcher):
    def generate_code(self, kwargs):
        ...
        if 'out' in kwargs and kwargs['out'] is not None:
            ...
            code = API_TEMPLATE.format(kwargs['input'], kwargs['dim'], kwargs['keepdim'], kwargs['out'], kwargs['input'], kwargs['dim'], kwargs['keepdim'], kwargs['out'])
            ...
            code = API_TEMPLATE.format(kwargs['input'], kwargs['dim'], kwargs['keepdim'], kwargs['input'], kwargs['dim'], kwargs['keepdim'])
...
class TensorAMinMaxMatcher(BaseMatcher):
    def get_paddle_class_nodes(self, func, args, kwargs):
        ...
        code = API_TEMPLATE.format(self.paddleClass, kwargs['dim'], kwargs['keepdim'], self.paddleClass, kwargs['dim'], kwargs['keepdim'])
