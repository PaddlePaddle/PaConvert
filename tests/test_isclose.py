...
def test_case_6():
    pytorch_code = textwrap.dedent(
        """
        import torch
        result = torch.isclose(torch.tensor([10000., 1e-07]), torch.tensor([10000.1, 1e-08]), atol=2., rtol=1e-05, equal_nan=False)
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_7():
    pytorch_code = textwrap.dedent(
        """
        import torch
        result = torch.isclose(torch.tensor([10000., 1e-07]), torch.tensor([10000.1, 1e-08]), 1e-05, 2., False)
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_8():
    pytorch_code = textwrap.dedent(
        """
        import torch
        result = torch.isclose(input=torch.tensor([10000., 1e-07]), other=torch.tensor([10000.1, 1e-08]), rtol=1e-05, atol=2., equal_nan=False)
        """
    )
    obj.run(pytorch_code, ["result"])
