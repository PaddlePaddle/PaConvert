...
def test_case_6():
    pytorch_code = textwrap.dedent(
        """
        import torch
        result = torch.randn_like(input=torch.zeros(3, 4, dtype=torch.float64), dtype=torch.float32 , layout=None, device="cpu", requires_grad=True, memory_format=torch.preserve_format)
        """
    )
    obj.run(pytorch_code, ["result"], check_value=False)
...
def test_case_7():
    pytorch_code = textwrap.dedent(
        """
        import torch
        result = torch.randn_like(memory_format=torch.preserve_format, requires_grad=True, device="cpu", layout=None, dtype=torch.float32, input=torch.zeros(3, 4, dtype=torch.float64))
        """
    )
    obj.run(pytorch_code, ["result"], check_value=False)
