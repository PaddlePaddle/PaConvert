...
def test_case_5():
    pytorch_code = textwrap.dedent(
        ...
        boundaries = torch.tensor([1, 3, 5, 7, 9])
        v = torch.tensor([[3, 6, 9], [3, 6, 9]])
        out = torch.tensor([[3, 6, 9], [3, 6, 9]], dtype=torch.int32)
        result = torch.bucketize(input=v, boundaries=boundaries, out_int32=True, right=True, out=out)
        """
    )
    obj.run(pytorch_code, ["out"])
...
def test_case_6():
    pytorch_code = textwrap.dedent(
        ...
        v = torch.tensor([[3, 6, 9], [3, 6, 9]])
        out = torch.tensor([[3, 6, 9], [3, 6, 9]], dtype=torch.int32)
        result = torch.bucketize(out=out, right=True, out_int32=True, boundaries=boundaries, input=v)
        """
    )
    obj.run(pytorch_code, ["out"])
