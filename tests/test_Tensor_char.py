...
obj = APIBase("torch.Tensor.char")


def test_all_type_to_char():
    dtypes = [
        "torch.float16",
        "torch.float32",
        "torch.float64",
    ...
    # for uint8 --> char, since torch.Tensor does not support converting negative
...
def test_complex64_to_char():
    # for complex --> char, we used the new test data
    pytorch_code = textwrap.dedent(
        """
        import torch
