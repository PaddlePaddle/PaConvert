...
def test_case_1():
    pytorch_code = textwrap.dedent(
        ...
        scheduler = torch.optim.lr_scheduler.StepLR(sgd, step_size=2, verbose=True)
...
def test_case_2():
    pytorch_code = textwrap.dedent(
        ...
        scheduler = torch.optim.lr_scheduler.StepLR(sgd, step_size=2, gamma=0.05)
        for epoch in range(1, 11):
            out = conv(inp)
            loss = torch.mean(out)
            loss.backward()
...
def test_case_3():
    pytorch_code = textwrap.dedent(
        ...
        sgd = torch.optim.SGD(conv.parameters(), lr=0.2)
        scheduler = torch.optim.lr_scheduler.StepLR(sgd, step_size=3, gamma=0.2)
        for epoch in range(1, 11):
            out = conv(inp)
            loss = torch.mean(out)
            loss.backward()
