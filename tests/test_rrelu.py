...
def test_case_6():
    pytorch_code = textwrap.dedent(
        ...
        x = torch.tensor([[[-1.3020, -0.1005,  0.5766,  0.6351, -0.8893,  0.0253, -0.1756, 1.2913],
                            [-0.8833, -0.1369, -0.0168, -0.5409, -0.1511, -0.1240, -1.1870, -1.8816]]])
        result = torch.rrelu(x, 0.02, 0.9, True)
        """
    )
    obj.run(pytorch_code, ["result"], check_value=False)
...
def test_case_7():
    pytorch_code = textwrap.dedent(
        ...
        x = torch.tensor([[[-1.3020, -0.1005,  0.5766,  0.6351, -0.8893,  0.0253, -0.1756, 1.2913],
                            [-0.8833, -0.1369, -0.0168, -0.5409, -0.1511, -0.1240, -1.1870, -1.8816]]])
        result = torch.rrelu(input=x, lower=0.02, upper=0.9, training=True)
        """
    )
    obj.run(pytorch_code, ["result"], check_value=False)
...
def test_case_8():
    pytorch_code = textwrap.dedent(
        ...
        x = torch.tensor([[[-1.3020, -0.1005,  0.5766,  0.6351, -0.8893,  0.0253, -0.1756, 1.2913],
                            [-0.8833, -0.1369, -0.0168, -0.5409, -0.1511, -0.1240, -1.1870, -1.8816]]])
        result = torch.rrelu(training=True, upper=0.9, lower=0.02, input=x)
        """
    )
    obj.run(pytorch_code, ["result"], check_value=False)
