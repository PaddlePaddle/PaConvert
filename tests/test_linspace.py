...
def test_case_7():
    pytorch_code = textwrap.dedent(
        ...
        import torch
        out = torch.tensor([1., 2, 3], dtype=torch.float64)
        result = torch.linspace(-1, 3, 4, dtype=torch.float64, out=out, layout=torch.strided, device="cpu", requires_grad=False)
        """
    )
    obj.run(pytorch_code, ["result", "out"])
...
def test_case_8():
    pytorch_code = textwrap.dedent(
        ...
        import torch
        out = torch.tensor([1., 2, 3], dtype=torch.float64)
        result = torch.linspace(start=-1, end=3, steps=4, out=out, dtype=torch.float64, layout=torch.strided, device="cpu", requires_grad=False)
        """
    )
    obj.run(pytorch_code, ["result", "out"])
