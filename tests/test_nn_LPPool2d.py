...
def test_case_1():
    pytorch_code = textwrap.dedent(
        ...
        pool = torch.nn.LPPool2d(1, 3, stride=2)
        result = pool(input)
        """
    )
...
def test_case_2():
    pytorch_code = textwrap.dedent(
        ...
        pool = torch.nn.LPPool2d(2, 4, stride=2)
        result = pool(input)
        """
    )
    obj.run(pytorch_code, ["result"], atol=1e-05, rtol=1e-06)
...
def test_case_3():
    pytorch_code = textwrap.dedent(
        ...
        input = torch.tensor([[[[-0.5743,  0.4889, -0.0878,  0.4210, -0.0844],
            ...
            [ 0.3789, -0.0560, -0.4090, -0.1070, -1.0139],
            [ 0.9204,  1.0817, -2.6126,  0.4244,  0.3272]]]])
        pool = torch.nn.LPPool2d(10, 3, stride=2, ceil_mode=True)
        result = pool(input)
        """
    )
    obj.run(pytorch_code, ["result"], atol=1e-05, rtol=1e-06)
...
def test_case_4():
    pytorch_code = textwrap.dedent(
        ...
        input = torch.tensor([[[[-0.5743,  0.4889, -0.0878,  0.4210, -0.0844],
            ...
            [ 0.3789, -0.0560, -0.4090, -0.1070, -1.0139],
            [ 0.9204,  1.0817, -2.6126,  0.4244,  0.3272]]]])
        pool = torch.nn.LPPool2d(10, 3, stride=4)
        result = pool(input)
        """
    )
    obj.run(pytorch_code, ["result"], atol=1e-05, rtol=1e-06)
...
def test_case_5():
    pytorch_code = textwrap.dedent(
        ...
        input = torch.tensor([[[[ 0.6430,  0.4511, -1.6757,  1.7116],
            ...
            [-0.0991, -0.5112, -0.1785,  2.0903],
            [-1.3286, -0.9333, -0.1404,  1.2582]]]])
        pool = nn.LPPool2d(norm_type=6, kernel_size=2, stride=2, ceil_mode=True)
        result = pool(input)
        """
    )
    obj.run(pytorch_code, ["result"], atol=1e-05, rtol=1e-06)
...
def test_case_6():
    pytorch_code = textwrap.dedent(
        ...
        input = torch.tensor([[[[ 0.6430,  0.4511, -1.6757,  1.7116],
            ...
            [-0.0991, -0.5112, -0.1785,  2.0903],
            [-1.3286, -0.9333, -0.1404,  1.2582]]]])
        pool = nn.LPPool2d(norm_type=float('inf'), kernel_size=2, stride=2, ceil_mode=True)
        result = pool(input)
        """
    )
    obj.run(pytorch_code, ["result"], atol=1e-05, rtol=1e-06)
...
def test_case_8():
    pytorch_code = textwrap.dedent(
        ...
        input = torch.tensor([[[[ 0.6430,  0.4511, -1.6757,  1.7116],
            ...
            [-0.0991, -0.5112, -0.1785,  2.0903],
            [-1.3286, -0.9333, -0.1404,  1.2582]]]])
        pool = nn.LPPool2d(kernel_size=2, stride=2, ceil_mode=True, norm_type=6)
        result = pool(input)
        """
    )
    obj.run(pytorch_code, ["result"], atol=1e-05, rtol=1e-06)
