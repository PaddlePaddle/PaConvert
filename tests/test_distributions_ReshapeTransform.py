...
def test_case_2():
    pytorch_code = textwrap.dedent(
        ...
        reshape_transform = torch.distributions.ReshapeTransform(in_shape=(2, 3), out_shape=(3, 2))
        result = reshape_transform.forward_shape((1,2,3))
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_3():
    pytorch_code = textwrap.dedent(
        ...
        reshape_transform = torch.distributions.ReshapeTransform(out_shape=(3, 2), in_shape=(2, 3))
        result = reshape_transform.forward_shape((1,2,3))
        """
    )
    obj.run(pytorch_code, ["result"])
