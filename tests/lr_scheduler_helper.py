...
def generate_torch_code(scheduler_init, step_with_loss=False):
    ...
    if isinstance(scheduler_init, str):
        scheduler_init = [scheduler_init]
        scheduler_names = ["scheduler"]
    else:
        scheduler_names = [f"scheduler_{i+1}" for i, _ in enumerate(scheduler_init)]

    train_segment_template = """
    {} = {}
    for epoch in range(1, 11):
        ...
        {}.step({})
    """
    step_args = "loss" if step_with_loss else ""
    ...
    return init_segment + train_segment + result_segment
