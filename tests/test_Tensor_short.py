...
obj = APIBase("torch.Tensor.short")


def test_all_type_to_short():
    dtypes = [
        "torch.float16",
        "torch.float32",
        "torch.float64",
    ...
    # for uint8 --> short, since torch.Tensor does not support converting negative
...
def test_complex64_to_short():
    # for complex --> short, we used the new test data
    pytorch_code = textwrap.dedent(
        """
        import torch
