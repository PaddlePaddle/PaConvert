...
def test_case_6():
    pytorch_code = textwrap.dedent(
        ...
        x = torch.tensor([[[-1.3020, -0.1005,  0.5766,  0.6351, -0.8893,  0.0253, -0.1756, 1.2913],
                            [-0.8833, -0.1369, -0.0168, -0.5409, -0.1511, -0.1240, -1.1870, -1.8816]]])
        result = F.alpha_dropout(x, 0.5, True, True)
        """
    )
    obj.run(pytorch_code, ["result"], check_value=False)
...
def test_case_7():
    pytorch_code = textwrap.dedent(
        ...
        x = torch.tensor([[[-1.3020, -0.1005,  0.5766,  0.6351, -0.8893,  0.0253, -0.1756, 1.2913],
                            [-0.8833, -0.1369, -0.0168, -0.5409, -0.1511, -0.1240, -1.1870, -1.8816]]])
        result = F.alpha_dropout(input=x, p=0.5, training=True, inplace=True)
        """
    )
    obj.run(pytorch_code, ["result"], check_value=False)
...
def test_case_8():
    pytorch_code = textwrap.dedent(
        ...
        x = torch.tensor([[[-1.3020, -0.1005,  0.5766,  0.6351, -0.8893,  0.0253, -0.1756, 1.2913],
                            [-0.8833, -0.1369, -0.0168, -0.5409, -0.1511, -0.1240, -1.1870, -1.8816]]])
        result = F.alpha_dropout(inplace=True, training=True, p=0.5, input=x)
        """
    )
    obj.run(pytorch_code, ["result"], check_value=False)
