...
def test_case_1():
    pytorch_code = textwrap.dedent(
        ...
        out = conv(inp)
        loss = torch.mean(out)
        adamw = torch.optim.AdamW(conv.parameters(), betas=(0.5, 0.99))
        loss.backward()
        adamw.step()
...
def test_case_2():
    pytorch_code = textwrap.dedent(
        ...
        out = conv(inp)
        loss = torch.mean(out)
        adamw = torch.optim.AdamW(conv.parameters(), eps=1e-7, lr=0.01)
        loss.backward()
        adamw.step()
        result = conv.weight.mean()
        """
...
def test_case_3():
    pytorch_code = textwrap.dedent(
        ...
        out = conv(inp)
        loss = torch.mean(out)
        adamw = torch.optim.AdamW(conv.parameters(), weight_decay=0.02)
        loss.backward()
        adamw.step()
        result = conv.weight.mean()
        """
