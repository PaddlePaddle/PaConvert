...
def test_case_8():
    pytorch_code = textwrap.dedent(
        ...
        input = torch.tensor([1., 2., 3.])
        out = torch.tensor([1.], dtype=torch.float64)
        result = torch.sum(input=input, dim=0, keepdim=True, dtype=torch.float64, out=out)
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_9():
    pytorch_code = textwrap.dedent(
        ...
        input = torch.tensor([1., 2., 3.])
        out = torch.tensor([1.], dtype=torch.float64)
        result = torch.sum(out=out, dtype=torch.float64, keepdim=True, dim=0, input=input)
        """
    )
    obj.run(pytorch_code, ["result"])
