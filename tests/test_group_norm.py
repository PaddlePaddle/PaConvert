...
def test_case_3():
    pytorch_code = textwrap.dedent(
        ...
        x = torch.tensor([[[[-1.2392, -0.1310, -0.6679,  0.5476],
                            ...
                            [ 1.1926, -0.0247,  0.4744, -1.0216],
                            [-0.0360, -1.1656,  0.3661, -1.8147]]]])
        result = torch.group_norm(x, 2, eps=1e-5)
        """
    )
    obj.run(pytorch_code, ["result"], atol=1e-4)
...
def test_case_4():
    pytorch_code = textwrap.dedent(
        ...
        result = torch.group_norm(x, 2, weight, bias, 1e-5)
        """
    )
    obj.run(pytorch_code, ["result"], atol=1e-4)
...
def test_case_5():
    pytorch_code = textwrap.dedent(
        ...
        weight = torch.tensor([1.3, 1.2])
        bias = torch.tensor([0.1, 0.2])
        result = torch.group_norm(input=x, num_groups=2, weight=weight, bias=bias, eps=1e-5)
        """
    )
    obj.run(pytorch_code, ["result"], atol=1e-4)
...
def test_case_6():
    pytorch_code = textwrap.dedent(
        ...
        weight = torch.tensor([1.3, 1.2])
        bias = torch.tensor([0.1, 0.2])
        result = torch.group_norm(eps=1e-5, bias=bias, weight=weight, num_groups=2, input=x)
        """
    )
    obj.run(pytorch_code, ["result"], atol=1e-4)
...
def test_case_8():
    pytorch_code = textwrap.dedent(
        ...
        result = torch.group_norm(x, 1, weight, bias, 1e-5)
        """
    )
    obj.run(pytorch_code, ["result"], atol=1e-4)
