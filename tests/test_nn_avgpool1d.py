...
obj = APIBase("torch.nn.AvgPool1d")
...
def test_case_1():
    pytorch_code = textwrap.dedent(
        ...
        pool = torch.nn.AvgPool1d(3, stride=2)
        result = pool(input)
        """
    )
...
def test_case_2():
    pytorch_code = textwrap.dedent(
        ...
        pool = torch.nn.AvgPool1d(4, stride=2, padding=2)
        result = pool(input)
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_3():
    pytorch_code = textwrap.dedent(
        ...
        input = torch.tensor([[[-0.5743,  0.4889, -0.0878,  0.4210, -0.0844],
            ...
            [ 0.3789, -0.0560, -0.4090, -0.1070, -1.0139],
            [ 0.9204,  1.0817, -2.6126,  0.4244,  0.3272]]])
        pool = torch.nn.AvgPool1d(3, stride=2, ceil_mode=True)
        result = pool(input)
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_4():
    pytorch_code = textwrap.dedent(
        ...
        input = torch.tensor([[[-0.5743,  0.4889, -0.0878,  0.4210, -0.0844],
            ...
            [ 0.3789, -0.0560, -0.4090, -0.1070, -1.0139],
            [ 0.9204,  1.0817, -2.6126,  0.4244,  0.3272]]])
        pool = torch.nn.AvgPool1d(3, stride=2, ceil_mode=True, count_include_pad=False)
        result = pool(input)
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_5():
    pytorch_code = textwrap.dedent(
        ...
        input = torch.tensor([[[ 0.6430,  0.4511, -1.6757,  1.7116],
            ...
            [-0.0991, -0.5112, -0.1785,  2.0903],
            [-1.3286, -0.9333, -0.1404,  1.2582]]])
        pool = nn.AvgPool1d(kernel_size=2, stride=2, padding=1, ceil_mode=True, count_include_pad=False)
        result = pool(input)
        """
    )
    obj.run(pytorch_code, ["result"])
