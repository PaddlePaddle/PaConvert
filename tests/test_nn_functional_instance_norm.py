...
def test_case_7():
    pytorch_code = textwrap.dedent(
        ...
        input = torch.tensor([[[ 1.1524,  0.4714,  0.2857],
                                ...
                                [ 0.4135, -0.2587,  0.0482]]])
        data = torch.tensor([1., 1., 1.])
        result = F.instance_norm(input=input, running_mean=data, running_var=data, weight=data, bias=data, use_input_stats=True, momentum=0.2, eps=1e-4)
        """
    )
    obj.run(pytorch_code, ["result"], rtol=1.0e-5, atol=1.0e-8)
