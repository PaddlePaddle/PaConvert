...
def test_case_6():
    pytorch_code = textwrap.dedent(
        """
        import torch
        result = torch.scalar_tensor(s=1., dtype=torch.float32, layout=torch.strided, device=torch.device('cpu'), requires_grad=True, pin_memory=False)
        """
    )
    obj.run(pytorch_code, ["result"])
