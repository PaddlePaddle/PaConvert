...
obj = DistributionAPIBase("torch.distributions.geometric.Geometric")
...
def test_case_3():
    pytorch_code = textwrap.dedent(
        """
        import torch
        m = torch.distributions.geometric.Geometric(0.5, validate_args=False)
        result = m.sample([100])
        """
    )
    obj.run(pytorch_code, ["result"], check_value=False)
...
def test_case_4():
    pytorch_code = textwrap.dedent(
        """
        import torch
        m = torch.distributions.geometric.Geometric(probs=0.5, validate_args=False)
        result = m.sample([100])
        """
    )
    obj.run(pytorch_code, ["result"], check_value=False)
...
def test_case_8():
    ...
    pytorch_code = textwrap.dedent(
        ...
        result = torch.distributions.geometric.Geometric(a)
        """
    )
