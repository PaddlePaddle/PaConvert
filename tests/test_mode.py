...
def test_case_3():
    pytorch_code = textwrap.dedent(
        ...
        import torch
        input = torch.tensor([[[1,2,2],[2,3,3]],[[0,5,5],[9,9,0]]])
        result, index = torch.mode(input, 1)
        """
    )
    obj.run(pytorch_code, ["result", "index"])
...
def test_case_4():
    pytorch_code = textwrap.dedent(
        ...
        import torch
        input = torch.tensor([[[1,2,2],[2,3,3]],[[0,5,5],[9,9,0]]])
        result, index = torch.mode(input, 1, keepdim=True)
        """
    )
    obj.run(pytorch_code, ["result", "index"])
...
def test_case_6():
    pytorch_code = textwrap.dedent(
        ...
        input = torch.tensor([[[1,2,2],[2,3,3]],[[0,5,5],[9,9,0]]])
        keepdim = True
        result, index = torch.mode(input=input, dim=1, keepdim=keepdim)
        """
    )
    obj.run(pytorch_code, ["result", "index"])
