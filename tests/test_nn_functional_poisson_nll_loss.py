...
def test_case_10():
    pytorch_code = textwrap.dedent(
        ...
        x1 = torch.tensor(a)
        x2 = torch.tensor(b)
        result = torch.nn.functional.poisson_nll_loss(x1, x2, log_input=True, full=False, size_average=None, eps=1e-08, reduce=None, reduction='mean')
        """
    )
    obj.run(pytorch_code, ["x1", "x2", "result"])
...
def test_case_11():
    pytorch_code = textwrap.dedent(
        ...
        x1 = torch.tensor(a)
        x2 = torch.tensor(b)
        result = torch.nn.functional.poisson_nll_loss(x1, x2, True, False, None, 1e-08, None, 'mean')
        """
    )
    obj.run(pytorch_code, ["x1", "x2", "result"])
...
def test_case_12():
    pytorch_code = textwrap.dedent(
        ...
        x1 = torch.tensor(a)
        x2 = torch.tensor(b)
        result = torch.nn.functional.poisson_nll_loss(input=x1, target=x2, log_input=True, full=False, size_average=None, eps=1e-08, reduce=None, reduction='mean')
        """
    )
    obj.run(pytorch_code, ["x1", "x2", "result"])
