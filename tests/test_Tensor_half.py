...
obj = APIBase("torch.Tensor.half")


def test_all_type_to_half():
    dtypes = [
        "torch.float16",
        "torch.float32",
        "torch.float64",
    ...
    # for uint8 --> half, since torch.Tensor does not support converting negative
...
def test_complex64_to_half():
    # for complex --> half, we used the new test data
    pytorch_code = textwrap.dedent(
        """
        import torch
