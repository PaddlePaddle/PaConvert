# Copyright (c) 2023 PaddlePaddle Authors. All Rights Reserved.
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

import textwrap

from apibase import APIBase

obj = APIBase("torch.nn.RNNBase")


def test_case_1():
    pytorch_code = textwrap.dedent(
        """
        import torch
        import torch.nn as nn

        model = torch.nn.RNNBase('LSTM', 16, 32)

        model_args = (model.input_size, model.hidden_size, model.num_layers)
        """
    )
    obj.run(pytorch_code, ["model_args"])


def test_case_2():
    pytorch_code = textwrap.dedent(
        """
        import torch
        import torch.nn as nn

        model = torch.nn.RNNBase('GRU', 16, 32)

        model_args = (model.input_size, model.hidden_size, model.num_layers)
        """
    )
    obj.run(pytorch_code, ["model_args"])


def test_case_3():
    pytorch_code = textwrap.dedent(
        """
        import torch
        import torch.nn as nn

        model = torch.nn.RNNBase('LSTM', 16, 32, bias=False)

        model_args = (model.input_size, model.hidden_size, model.num_layers)
        """
    )
    obj.run(pytorch_code, ["model_args"])


def test_case_4():
    pytorch_code = textwrap.dedent(
        """
        import torch
        import torch.nn as nn

        model = torch.nn.RNNBase('LSTM', 16, 32, batch_first=True)

        model_args = (model.input_size, model.hidden_size, model.num_layers)
        """
    )
    obj.run(pytorch_code, ["model_args"])


def test_case_5():
    pytorch_code = textwrap.dedent(
        """
        import torch
        import torch.nn as nn

        model = torch.nn.RNNBase('LSTM', 16, 32, bidirectional=True)

        model_args = (model.input_size, model.hidden_size, model.num_layers)
        """
    )
    obj.run(pytorch_code, ["model_args"])


def test_case_6():
    pytorch_code = textwrap.dedent(
        """
        import torch
        import torch.nn as nn

        model = torch.nn.RNNBase('LSTM', 16, 32, bidirectional=False)

        model_args = (model.input_size, model.hidden_size, model.num_layers)
        """
    )
    obj.run(pytorch_code, ["model_args"])


def test_case_7():
    pytorch_code = textwrap.dedent(
        """
        import torch
        import torch.nn as nn

        model = torch.nn.RNNBase(mode='RNN_RELU', input_size=16, hidden_size=32, num_layers=2, bias=True, batch_first=False, dropout=0.0, bidirectional=False, proj_size=0, device='cpu', dtype=None)

        model_args = (model.input_size, model.hidden_size, model.num_layers)
        """
    )
    obj.run(pytorch_code, ["model_args"])


def test_case_8():
    pytorch_code = textwrap.dedent(
        """
        import torch
        import torch.nn as nn

        model = torch.nn.RNNBase('RNN_RELU', 16, 32, 2, True, False, 0.0, False, 0, 'cpu', None)

        model_args = (model.input_size, model.hidden_size, model.num_layers)
        """
    )
    obj.run(pytorch_code, ["model_args"])


def test_case_9():
    pytorch_code = textwrap.dedent(
        """
        import torch
        import torch.nn as nn

        model = torch.nn.RNNBase(mode='RNN_RELU', hidden_size=32, batch_first=False, num_layers=2, bias=True, input_size=16, dropout=0.0, bidirectional=False, proj_size=0, device='cpu', dtype=None)

        model_args = (model.input_size, model.hidden_size, model.num_layers)
        """
    )
    obj.run(pytorch_code, ["model_args"])


def test_mode_case_1():
    pytorch_code = textwrap.dedent(
        """
        import torch
        import torch.nn as nn

        model = torch.nn.RNNBase('RNN_TANH', 16, 32, bidirectional=False)

        model_args = (model.input_size, model.hidden_size, model.num_layers)
        """
    )
    obj.run(
        pytorch_code,
        ["model_args"],
    )


def test_mode_case_2():
    pytorch_code = textwrap.dedent(
        """
        import torch
        import torch.nn as nn

        model = torch.nn.RNNBase('RNN_RELU', 16, 32, bidirectional=False)

        model_args = (model.input_size, model.hidden_size, model.num_layers)
        """
    )
    obj.run(
        pytorch_code,
        ["model_args"],
    )
