...
def generate_torch_code(optim_seg):
    code_template = f"""
        import torch
...
def test_case_1():
    pytorch_code = textwrap.dedent(
        generate_torch_code("torch.optim.Adadelta(conv.parameters())")
    )
...
def test_case_2():
    pytorch_code = textwrap.dedent(
        generate_torch_code("torch.optim.Adadelta(conv.parameters(),weight_decay=0.01)")
    )
    obj.run(pytorch_code, ["result"], rtol=1.0e-5)
