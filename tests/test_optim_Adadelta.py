...
def test_case_1():
    pytorch_code = textwrap.dedent(
        ...
        out = conv(inp)
        loss = torch.mean(out)
        adadelta = torch.optim.Adadelta(conv.parameters())
        loss.backward()
        adadelta.step()
...
def test_case_2():
    pytorch_code = textwrap.dedent(
        ...
        out = conv(inp)
        loss = torch.mean(out)
        adadelta = torch.optim.Adadelta(conv.parameters(),weight_decay=0.01)
        loss.backward()
        adadelta.step()
        result = conv.weight.mean()
        """
...
def test_case_3():
    pytorch_code = textwrap.dedent(
        ...
        out = conv(inp)
        loss = torch.mean(out)
        adadelta = torch.optim.Adadelta(conv.parameters(),weight_decay=0.01,lr=0.1)
        loss.backward()
        adadelta.step()
        result = conv.weight.mean()
        """
