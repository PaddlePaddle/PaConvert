...
class Block(nn.Module):
    def __init__(
        ...
        if grow_first:
            ...
            rep.append(
                SeparableConv2d(
                    in_channels, out_channels, 3, 1, dilation, norm_layer=norm_layer
                )
            )
        ...
        for i in range(reps - 1):
            if grow_first or start_with_relu:
                rep.append(self.relu)
            rep.append(
                SeparableConv2d(filters, filters, 3, 1, dilation, norm_layer=norm_layer)
            )
        ...
        if stride != 1:
            ...
            rep.append(
                SeparableConv2d(
                    out_channels, out_channels, 3, 1, dilation, norm_layer=norm_layer
                )
            )
            rep.append(norm_layer(out_channels))
...
class Xception65(nn.Module):
    ...
    def __init__(self, num_classes=1000, output_stride=32, norm_layer=nn.BatchNorm2d):
        ...
        self.block1 = Block(
            64, 128, reps=2, stride=2, norm_layer=norm_layer, start_with_relu=False
        )
        self.block2 = Block(
            128,
            256,
