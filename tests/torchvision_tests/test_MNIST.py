...
def test_case_2():
    ...
    paddle_code = textwrap.dedent(
        ...
        train_dataset = paddle.vision.datasets.MNIST(mode='train' if train else
            'test', image_path=Path(root_path) /
            'MNIST/raw/train-images-idx3-ubyte.gz' if train else Path(root_path) /
            'MNIST/raw/t10k-images-idx3-ubyte.gz', label_path=Path(root_path) /
            'MNIST/raw/train-labels-idx1-ubyte.gz' if train else Path(root_path) /
...
def test_case_3():
    ...
    paddle_code = textwrap.dedent(
        ...
        train_dataset = paddle.vision.datasets.MNIST(download=download, mode=
            'train' if train else 'test', image_path=Path(root_path) /
            'MNIST/raw/train-images-idx3-ubyte.gz' if train else Path(root_path) /
            'MNIST/raw/t10k-images-idx3-ubyte.gz', label_path=Path(root_path) /
            'MNIST/raw/train-labels-idx1-ubyte.gz' if train else Path(root_path) /
            'MNIST/raw/t10k-labels-idx1-ubyte.gz')
