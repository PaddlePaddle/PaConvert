...
obj = APIBase("torchvision.ops.RoIPool")
...
def test_case_4():
    pytorch_code = textwrap.dedent(
        ...
        import torch
        from torchvision.ops import RoIPool
        roi_pool = RoIPool(output_size=7, spatial_scale=1.0)
        """
    )
    paddle_code = textwrap.dedent(
        """
        import paddle
        roi_pool = paddle.vision.ops.RoIPool(output_size=7, spatial_scale=1.0)
        """
    )
    obj.run(pytorch_code, expect_paddle_code=paddle_code, check_value=False)
...
def test_case_5():
    pytorch_code = textwrap.dedent(
        ...
        import torch
        from torchvision.ops import RoIPool
        roi_pool = RoIPool(output_size=(7, 7), spatial_scale=2.0)
        """
    )
    paddle_code = textwrap.dedent(
        """
        import paddle
        roi_pool = paddle.vision.ops.RoIPool(output_size=(7, 7), spatial_scale=2.0)
        """
    )
    obj.run(pytorch_code, expect_paddle_code=paddle_code, check_value=False)
