...
def test_case_1():
    pytorch_code = textwrap.dedent(
        ...
        out = conv(inp)
        loss = torch.mean(out)
        adam = torch.optim.Adam(conv.parameters(), eps=1e-7)
        loss.backward()
        adam.step()
...
def test_case_2():
    pytorch_code = textwrap.dedent(
        ...
        out = conv(inp)
        loss = torch.mean(out)
        adam = torch.optim.Adam(conv.parameters(), betas=(0.5, 0.99))
        loss.backward()
        adam.step()
        result = conv.weight.mean()
        """
...
def test_case_3():
    pytorch_code = textwrap.dedent(
        ...
        out = conv(inp)
        loss = torch.mean(out)
        adam = torch.optim.Adam(conv.parameters(), weight_decay=0.01)
        loss.backward()
        adam.step()
        result = conv.weight.mean()
        """
