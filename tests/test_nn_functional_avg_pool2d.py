...
def test_case_5():
    pytorch_code = textwrap.dedent(
        ...
        input = torch.tensor([[[[ 0.6430,  0.4511, -1.6757,  1.7116],
            ...
            [-0.0991, -0.5112, -0.1785,  2.0903],
            [-1.3286, -0.9333, -0.1404,  1.2582]]]])
        result = F.avg_pool2d(input=input, kernel_size=2, stride=2, padding=1, ceil_mode=True, count_include_pad=False, divisor_override=None)
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_6():
    pytorch_code = textwrap.dedent(
        ...
        input = torch.tensor([[[[ 0.6430,  0.4511, -1.6757,  1.7116],
            ...
            [-0.0991, -0.5112, -0.1785,  2.0903],
            [-1.3286, -0.9333, -0.1404,  1.2582]]]])
        result = F.avg_pool2d(input, 2, 2, 1, True, False, None)
        """
    )
    obj.run(pytorch_code, ["result"])
