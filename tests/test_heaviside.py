...
def test_case_6():
    pytorch_code = textwrap.dedent(
        ...
        values = torch.tensor([0.5])
        out = torch.tensor([-1.5, 0, 2.0])
        torch.heaviside(input=input, values=values, out=out)
        """
    )
    obj.run(pytorch_code, ["out"])
...
def test_case_7():
    pytorch_code = textwrap.dedent(
        ...
        values = torch.tensor([0.5])
        out = torch.tensor([-1.5, 0, 2.0])
        torch.heaviside(out=out, values=values, input=input)
        """
    )
    obj.run(pytorch_code, ["out"])
...
def test_case_8():
    pytorch_code = textwrap.dedent(
        ...
        values = torch.tensor([0.5])
        out = torch.tensor([-1.5, 0, 2.0])
        torch.heaviside(input=torch.tensor([-1.5, 0, 2.0]), values=values, out=out)
        """
    )
    obj.run(pytorch_code, ["out"])
...
def test_case_9():
    pytorch_code = textwrap.dedent(
        ...
        values = torch.tensor([0.5])
        out = torch.tensor([-1.5, 0, 2.0])
        torch.heaviside(out=out, values=values, input=torch.tensor([-1.5, 0, 2.0]))
        """
    )
    obj.run(pytorch_code, ["out"])
