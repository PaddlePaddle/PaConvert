...
def test_case_7():
    pytorch_code = textwrap.dedent(
        """
        import torch
        m = torch.distributions.Multinomial(total_count=1, probs=torch.tensor([0.3]), logits=torch.tensor([0.3]), validate_args=False)
        result = m.sample([100])
        """
    )
    obj.run(
...
def test_case_8():
    pytorch_code = textwrap.dedent(
        """
        import torch
        m = torch.distributions.Multinomial(total_count=1, logits=torch.tensor([0.3]), probs=torch.tensor([0.3]), validate_args=False)
        result = m.sample([100])
        """
    )
    obj.run(
