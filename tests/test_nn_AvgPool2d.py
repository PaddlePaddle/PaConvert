...
def test_case_6():
    pytorch_code = textwrap.dedent(
        ...
        input = torch.tensor([[[[ 0.6430,  0.4511, -1.6757,  1.7116],
            ...
            [-0.0991, -0.5112, -0.1785,  2.0903],
            [-1.3286, -0.9333, -0.1404,  1.2582]]]])
        pool = nn.AvgPool2d(kernel_size=2, stride=2, padding=1, ceil_mode=True, count_include_pad=False, divisor_override=None)
        result = pool(input)
        """
    )
    obj.run(pytorch_code, ["result"], atol=1e-05, rtol=1e-06)
...
def test_case_7():
    pytorch_code = textwrap.dedent(
        ...
        input = torch.tensor([[[[ 0.6430,  0.4511, -1.6757,  1.7116],
            ...
            [-0.0991, -0.5112, -0.1785,  2.0903],
            [-1.3286, -0.9333, -0.1404,  1.2582]]]])
        pool = nn.AvgPool2d(2, 2, 1, True, False, None)
        result = pool(input)
        """
    )
    obj.run(pytorch_code, ["result"], atol=1e-05, rtol=1e-06)
...
def test_case_8():
    pytorch_code = textwrap.dedent(
        ...
        input = torch.tensor([[[[ 0.6430,  0.4511, -1.6757,  1.7116],
            ...
            [-0.0991, -0.5112, -0.1785,  2.0903],
            [-1.3286, -0.9333, -0.1404,  1.2582]]]])
        pool = nn.AvgPool2d(divisor_override=None, count_include_pad=False, ceil_mode=True, padding=1, stride=2, kernel_size=2)
        result = pool(input)
        """
    )
    obj.run(pytorch_code, ["result"], atol=1e-05, rtol=1e-06)
