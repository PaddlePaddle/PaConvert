...
def test_case_6():
    pytorch_code = textwrap.dedent(
        """
        import torch
        result = torch.full_like(torch.empty(2, 3), 6, layout=torch.strided, dtype=torch.float64, device="cpu", requires_grad=True, memory_format=torch.preserve_format)
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_7():
    pytorch_code = textwrap.dedent(
        """
        import torch
        result = torch.full_like(input=torch.empty(2, 3), fill_value=6, dtype=torch.float64, layout=torch.strided, device="cpu", requires_grad=True, memory_format=torch.preserve_format)
        """
    )
    obj.run(pytorch_code, ["result"])
