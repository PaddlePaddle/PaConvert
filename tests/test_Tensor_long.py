...
obj = APIBase("torch.Tensor.long")


def test_all_type_to_long():
    dtypes = [
        "torch.float16",
        "torch.float32",
        "torch.float64",
    ...
    # for uint8 --> long, since torch.Tensor does not support converting negative
...
def test_complex64_to_long():
    # for complex --> long, we used the new test data
    pytorch_code = textwrap.dedent(
        """
        import torch
