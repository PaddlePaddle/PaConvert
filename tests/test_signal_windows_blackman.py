...
def test_case_10():
    pytorch_code = textwrap.dedent(
        """
        import torch
        result = torch.signal.windows.blackman(5, sym=False, dtype=torch.float64, requires_grad=True)
        """
    )
    obj.run(pytorch_code, ["result"], check_value=False)
...
def test_case_11():
    pytorch_code = textwrap.dedent(
        """
        import torch
        result = torch.signal.windows.blackman(5, sym=False, dtype=torch.float64, layout=torch.strided, requires_grad=True)
        """
    )
    obj.run(pytorch_code, ["result"], check_value=False)
...
def test_case_12():
    pytorch_code = textwrap.dedent(
        """
        import torch
        result = torch.signal.windows.blackman(5, sym=False, dtype=torch.float64, layout=torch.strided, device=torch.device('cpu'), requires_grad=True)
        """
    )
    obj.run(pytorch_code, ["result"], check_value=False)
