...
def test_case_5():
    pytorch_code = textwrap.dedent(
        """
        import torch
        m = torch.distributions.geometric.Geometric(probs=0.3, validate_args=False)
        result = m.sample([100])
        """
    )
    obj.run(pytorch_code, ["result"], check_value=False)
...
def test_case_6():
    pytorch_code = textwrap.dedent(
        """
        import torch
        m = torch.distributions.geometric.Geometric(validate_args=False, probs=0.3)
        result = m.sample([100])
        """
    )
    obj.run(pytorch_code, ["result"], check_value=False)
...
def test_case_9():
    pytorch_code = textwrap.dedent(
        """
        import torch
        m = torch.distributions.Geometric(torch.tensor([0.3]), None, False)
        result = m.sample([100])
        """
    )
    obj.run(
...
def test_case_10():
    pytorch_code = textwrap.dedent(
        """
        import torch
        m = torch.distributions.Geometric(probs=None, logits=15, validate_args=True)
        result = m.sample([100])
        """
    )
    obj.run(
