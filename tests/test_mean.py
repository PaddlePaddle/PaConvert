...
def test_case_3():
    pytorch_code = textwrap.dedent(
        ...
        import torch
        input = torch.tensor([[1.4907, 1.0593, 1.5696], [1.4907, 1.0593, 1.5696]])
        result = torch.mean(input, 1, keepdim=True)
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_4():
    pytorch_code = textwrap.dedent(
        ...
        import torch
        input = torch.tensor([[1.4907, 1.0593, 1.5696], [1.4907, 1.0593, 1.5696]])
        result = torch.mean(input, dim=1, keepdim=True)
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_5():
    pytorch_code = textwrap.dedent(
        ...
        result = torch.mean(input, dim=1, keepdim=True, out=out)
        """
    )
    obj.run(pytorch_code, ["result"])
