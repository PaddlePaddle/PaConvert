...
def test_case_9():
    pytorch_code = textwrap.dedent(
        ...
        b = torch.tensor([1., 1., 1., 0.])
        out = torch.tensor([1., 1, 1, 0])
        result = torch.sub(input=a, other=b, alpha=3, out=out)
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_10():
    pytorch_code = textwrap.dedent(
        ...
        b = torch.tensor([1., 1., 1., 0.])
        out = torch.tensor([1., 1, 1, 0])
        result = torch.sub(out=out, alpha=3, other=b, input=a)
        """
    )
    obj.run(pytorch_code, ["result"])
...
def _test_case_11():
    pytorch_code = textwrap.dedent(
        ...
        a = torch.tensor([ 0.5950,-0.0872, 2.3298, -0.2972])
        out = torch.tensor([1., 1, 1, 0])
        result = torch.sub(input=a, other=0.5, alpha=3, out=out)
        """
    )
    obj.run(pytorch_code, ["out"])
...
def _test_case_12():
    pytorch_code = textwrap.dedent(
        ...
        a = torch.tensor([ 0.5950,-0.0872, 2.3298, -0.2972])
        out = torch.tensor([1., 1, 1, 0])
        result = torch.sub(out=out, alpha=3, other=0.5, input=a)
        """
    )
    obj.run(pytorch_code, ["out"])
