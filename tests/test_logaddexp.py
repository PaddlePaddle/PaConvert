...
# paddle.logaddexp does not support type promotion between int and float.
def _test_case_5():
    pytorch_code = textwrap.dedent(
        """
        import torch
...
def test_case_6():
    pytorch_code = textwrap.dedent(
        ...
        a = torch.tensor([1., 2., -3., -4., 5.])
        out = torch.tensor([1., 2., -3., -4., 5.])
        result = torch.logaddexp(input=a, other=a, out=out)
        """
    )
    obj.run(pytorch_code, ["out"])
...
def test_case_7():
    pytorch_code = textwrap.dedent(
        ...
        a = torch.tensor([1., 2., -3., -4., 5.])
        out = torch.tensor([1., 2., -3., -4., 5.])
        result = torch.logaddexp(out=out, other=a, input=a)
        """
    )
    obj.run(pytorch_code, ["out"])
