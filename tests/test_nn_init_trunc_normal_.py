...
def test_case_8():
    pytorch_code = textwrap.dedent(
        ...
        torch.nn.init.trunc_normal_(tensor=linear.weight, mean=0., std=3., a=-5., b=5.)
...
def test_case_9():
    pytorch_code = textwrap.dedent(
        ...
        import torch
        linear = torch.nn.Linear(128, 256)
        torch.nn.init.trunc_normal_(tensor=linear.weight, a=-5., b=5.)
        result = linear.weight
        """
    )
    obj.run(pytorch_code, ["result"], check_value=False)
...
def test_case_10():
    pytorch_code = textwrap.dedent(
        ...
        import torch
        linear = torch.nn.Linear(128, 256)
        torch.nn.init.trunc_normal_(tensor=linear.weight,  mean=0.4, a=-5., )
        result = linear.weight
        """
    )
    obj.run(pytorch_code, ["result"], check_value=False)
