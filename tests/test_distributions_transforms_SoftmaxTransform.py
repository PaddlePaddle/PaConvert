...
def test_case_2():
    pytorch_code = textwrap.dedent(
        """
        import torch
        t = torch.distributions.transforms.SoftmaxTransform(cache_size=1)
        x = torch.tensor([1.0, 2.0, 3.0])
        result = t(x)
        """
    )
...
def test_case_3():
    pytorch_code = textwrap.dedent(
        """
        import torch
        t = torch.distributions.transforms.SoftmaxTransform(1)
        x = torch.tensor([1.0, 2.0, 3.0])
        result = t(x)
        """
    )
...
@pytest.mark.skip(
    reason="Paddle SoftmaxTransform does not support .inv() method, PyTorch supports it"
)
def test_case_5():
