...
def test_case_9():
    pytorch_code = textwrap.dedent(
        ...
        out = torch.tensor([[3, 6, 9],
                            [3, 6, 9]])
        sorter = torch.argsort(x)
        result = torch.searchsorted(x, values, out_int32=False, right=True, side='right', out=out, sorter=sorter)
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_10():
    pytorch_code = textwrap.dedent(
        ...
        out = torch.tensor([[3, 6, 9],
                            [3, 6, 9]])
        sorter = torch.argsort(x)
        result = torch.searchsorted(sorted_sequence=x, values=values, out_int32=False, right=True, side='right', out=out, sorter=sorter)
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_11():
    pytorch_code = textwrap.dedent(
        ...
        out = torch.tensor([[3, 6, 9],
                            [3, 6, 9]])
        sorter = torch.argsort(x)
        result = torch.searchsorted(sorter=sorter, out=out, side='right', right=True, out_int32=False, values=values, sorted_sequence=x)
        """
    )
    obj.run(pytorch_code, ["result"])
