...
def test_case_17():
    pytorch_code = textwrap.dedent(
        ...
        import torch
        a=False
        result = torch.signal.windows.hann(dtype=torch.float32, layout=torch.strided, device=torch.device('cpu'), M=5, sym=a, requires_grad=False)
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_18():
    pytorch_code = textwrap.dedent(
        ...
        a=False
        m=5
        result = torch.signal.windows.hann(dtype=torch.float32, M=m, sym=a, layout=torch.strided, device=torch.device('cpu'), requires_grad=False)
        """
    )
    obj.run(pytorch_code, ["result"])
