...
def test_case_10():
    pytorch_code = textwrap.dedent(
        ...
        input = torch.Tensor([[1, -2, 3], [0, -1, 2], [1, 0, 1]]).type(torch.float32)
        label = torch.Tensor([[-1, 1, -1], [1, 1, 1], [1, -1, 1]]).type(torch.float32)
        cri = torch.nn.HingeEmbeddingLoss(0.5, True, False, 'mean')
        result = cri(input, label)
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_11():
    pytorch_code = textwrap.dedent(
        ...
        input = torch.Tensor([[1, -2, 3], [0, -1, 2], [1, 0, 1]]).type(torch.float32)
        label = torch.Tensor([[-1, 1, -1], [1, 1, 1], [1, -1, 1]]).type(torch.float32)
        cri = torch.nn.HingeEmbeddingLoss(margin=0.5, size_average=True, reduce=False, reduction='mean')
        result = cri(input, label)
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_12():
    pytorch_code = textwrap.dedent(
        ...
        input = torch.Tensor([[1, -2, 3], [0, -1, 2], [1, 0, 1]]).type(torch.float32)
        label = torch.Tensor([[-1, 1, -1], [1, 1, 1], [1, -1, 1]]).type(torch.float32)
        cri = torch.nn.HingeEmbeddingLoss(0.7, False, False, 'mean')
        result = cri(input, label)
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_13():
    pytorch_code = textwrap.dedent(
        ...
        input = torch.Tensor([[1, -2, 3], [0, -1, 2], [1, 0, 1]]).type(torch.float32)
        label = torch.Tensor([[-1, 1, -1], [1, 1, 1], [1, -1, 1]]).type(torch.float32)
        cri = torch.nn.HingeEmbeddingLoss(margin=0.7, size_average=False, reduce=False, reduction='mean')
        result = cri(input, label)
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_20():
    pytorch_code = textwrap.dedent(
        ...
        input = torch.Tensor([[1, -2, 3], [0, -1, 2], [1, 0, 1]]).type(torch.float32)
        label = torch.Tensor([[-1, 1, -1], [1, 1, 1], [1, -1, 1]]).type(torch.float32)
        cri = torch.nn.HingeEmbeddingLoss(0.7, True, False, 'mean')
        result = cri(input, label)
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_21():
    pytorch_code = textwrap.dedent(
        ...
        input = torch.Tensor([[1, -2, 3], [0, -1, 2], [1, 0, 1]]).type(torch.float32)
        label = torch.Tensor([[-1, 1, -1], [1, 1, 1], [1, -1, 1]]).type(torch.float32)
        cri = torch.nn.HingeEmbeddingLoss(margin=0.7, size_average=True, reduce=False, reduction='mean')
        result = cri(input, label)
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_22():
    pytorch_code = textwrap.dedent(
        ...
        input = torch.Tensor([[1, -2, 3], [0, -1, 2], [1, 0, 1]]).type(torch.float32)
        label = torch.Tensor([[-1, 1, -1], [1, 1, 1], [1, -1, 1]]).type(torch.float32)
        cri = torch.nn.HingeEmbeddingLoss(0.7, True, True, 'mean')
        result = cri(input, label)
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_23():
    pytorch_code = textwrap.dedent(
        ...
        input = torch.Tensor([[1, -2, 3], [0, -1, 2], [1, 0, 1]]).type(torch.float32)
        label = torch.Tensor([[-1, 1, -1], [1, 1, 1], [1, -1, 1]]).type(torch.float32)
        cri = torch.nn.HingeEmbeddingLoss(margin=0.7, size_average=True, reduce=True, reduction='mean')
        result = cri(input, label)
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_24():
    pytorch_code = textwrap.dedent(
        ...
        input = torch.Tensor([[1, -2, 3], [0, -1, 2], [1, 0, 1]]).type(torch.float32)
        label = torch.Tensor([[-1, 1, -1], [1, 1, 1], [1, -1, 1]]).type(torch.float32)
        cri = torch.nn.HingeEmbeddingLoss(0.7, False, False, 'sum')
        result = cri(input, label)
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_25():
    pytorch_code = textwrap.dedent(
        ...
        input = torch.Tensor([[1, -2, 3], [0, -1, 2], [1, 0, 1]]).type(torch.float32)
        label = torch.Tensor([[-1, 1, -1], [1, 1, 1], [1, -1, 1]]).type(torch.float32)
        cri = torch.nn.HingeEmbeddingLoss(margin=0.7, size_average=False, reduce=False, reduction='sum')
        result = cri(input, label)
        """
    )
    obj.run(pytorch_code, ["result"])
