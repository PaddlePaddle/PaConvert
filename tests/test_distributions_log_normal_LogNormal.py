...
def test_case_3():
    pytorch_code = textwrap.dedent(
        """
        import torch
        m = torch.distributions.log_normal.LogNormal(torch.tensor([1.0]), torch.tensor([0.5]), validate_args=False)
        result = m.sample([100])
        """
    )
    obj.run(pytorch_code, ["result"], check_value=False)
...
def test_case_5():
    ...
    pytorch_code = textwrap.dedent(
        """
        import torch
        m = torch.distributions.log_normal.LogNormal(scale=torch.tensor([1.0]), loc=torch.tensor([0.0]), validate_args=False)
        result = m.sample([50])
        """
    )
    obj.run(pytorch_code, ["result"], check_value=False)
...
def test_case_6():
    ...
    pytorch_code = textwrap.dedent(
        """
        import torch
        m = torch.distributions.log_normal.LogNormal(torch.tensor([0.0]), scale=torch.tensor([0.5]))
        result = m.sample([30])
        """
    )
    obj.run(pytorch_code, ["result"], check_value=False)
