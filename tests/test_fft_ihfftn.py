...
def test_case_7():
    pytorch_code = textwrap.dedent(
        ...
        result = torch.fft.ihfftn(t, s=(2, 5), dim=(0, 1), norm='backward', out=out)
        """
    )
...
def test_case_8():
    pytorch_code = textwrap.dedent(
        ...
        import torch
        t = torch.arange(20).reshape((4, 5)).type(torch.float64)
        result = torch.fft.ihfftn(t, (2, 5), (0, 1), 'backward')
        """
    )
    obj.run(pytorch_code, ["result"], rtol=1.0e-5, atol=1.0e-8)
...
def test_case_9():
    pytorch_code = textwrap.dedent(
        ...
        t = torch.arange(20).reshape((4, 5)).type(torch.float64)
        out = torch.complex(torch.randn(2, 3, dtype=torch.float64), torch.randn(2, 3, dtype=torch.float64))
        result = torch.fft.ihfftn(input=t, s=(2, 5), dim=(0, 1), norm='backward', out=out)
        """
    )
    obj.run(pytorch_code, ["result", "out"], rtol=1.0e-5, atol=1.0e-8)
...
def test_case_10():
    pytorch_code = textwrap.dedent(
        ...
        t = torch.arange(20).reshape((4, 5)).type(torch.float64)
        out = torch.complex(torch.randn(2, 3, dtype=torch.float64), torch.randn(2, 3, dtype=torch.float64))
        result = torch.fft.ihfftn(out=out, norm='backward', dim=(0, 1), s=(2, 5), input=t)
        """
    )
    obj.run(pytorch_code, ["result", "out"], rtol=1.0e-5, atol=1.0e-8)
