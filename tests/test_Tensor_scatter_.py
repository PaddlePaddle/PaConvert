...
def test_case_4():
    pytorch_code = textwrap.dedent(
        ...
        import torch
        index = torch.tensor([[0],[1],[2]])
        result = torch.ones(3, 5).scatter_(dim=1, index=index, value=1.0, reduce='multiply')
...
def test_case_7():
    pytorch_code = textwrap.dedent(
        ...
        result = torch.ones(3, 5).scatter_(1, index, src)
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_8():
    pytorch_code = textwrap.dedent(
        ...
        index = torch.tensor([[0],[1],[2]])
        src = torch.tensor(src_np)
        result = torch.ones(3, 5).scatter_(dim=1, index=index, src=src)
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_9():
    pytorch_code = textwrap.dedent(
        ...
        index = torch.tensor([[0],[1],[2]])
        src = torch.tensor(src_np)
        result = torch.ones(3, 5).scatter_(1, index, src, reduce='add')
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_11():
    pytorch_code = textwrap.dedent(
        ...
        index = torch.tensor([[0],[1],[2]])
        src = torch.tensor(src_np)
        result = torch.ones(3, 5).scatter_(1, index, src, reduce='multiply')
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_12():
    pytorch_code = textwrap.dedent(
        ...
        index = torch.tensor([[0],[1],[2]])
        src = torch.tensor(src_np)
        result = torch.ones(3, 5).scatter_(dim=1, index=index, src=src, reduce='multiply')
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_19_complex():
    pytorch_code = textwrap.dedent(
        ...
        result = (input + input * input).scatter_(dim=1, index=index, src=src)
...
def test_case_20_complex():
    pytorch_code = textwrap.dedent(
        ...
        src = torch.zeros_like(index).to(input.dtype)
        src.requires_grad = True
        result = (input + input * input).scatter_(0, index=index, src=src)
        result.backward(torch.ones_like(result))
        input_grad = input.grad
        src_grad = src.grad
        input_grad.requires_grad = False
...
def test_case_21_complex():
    pytorch_code = textwrap.dedent(
        ...
        src.requires_grad = True
        result = (input + 1).scatter_(2, index, src)
        result.backward(torch.ones_like(result))
        input_grad = input.grad
        src_grad = src.grad
        input_grad.requires_grad = False
...
def test_case_22_complex():
    pytorch_code = textwrap.dedent(
        ...
        input.scatter_(3, index, src=src, reduce = "add")
        """
    )
...
def test_case_23_complex():
    pytorch_code = textwrap.dedent(
        ...
        input.scatter_(2, index, src=src, reduce = "multiply")
        """
    )
    obj.run(pytorch_code, ["input"])
