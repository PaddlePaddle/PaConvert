...
def test_case_6():
    pytorch_code = textwrap.dedent(
        """
        import torch
        result = torch.empty_like(torch.empty(2, 3), dtype=torch.float64, layout=torch.strided, device=None, requires_grad=True, memory_format=torch.preserve_format)
        """
    )
    obj.run(pytorch_code, ["result"], check_value=False)
...
def test_case_7():
    pytorch_code = textwrap.dedent(
        """
        import torch
        result = torch.empty_like(input=torch.empty(2, 3), dtype=torch.float64, layout=torch.strided, device=None, requires_grad=True, memory_format=torch.preserve_format)
        """
    )
    obj.run(pytorch_code, ["result"], check_value=False)
