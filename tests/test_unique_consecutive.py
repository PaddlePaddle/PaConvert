...
def test_case_6():
    pytorch_code = textwrap.dedent(
        ...
        import torch
        x = torch.tensor([1, 1, 2, 2, 3, 1, 1, 2])
        result, inverse_indices, counts = torch.unique_consecutive(input=x, return_inverse=True, return_counts=True, dim=0)
        """
    )
    obj.run(pytorch_code, ["result", "counts", "inverse_indices"])
...
def test_case_7():
    pytorch_code = textwrap.dedent(
        ...
        import torch
        x = torch.tensor([1, 1, 2, 2, 3, 1, 1, 2])
        result, inverse_indices, counts = torch.unique_consecutive(x, True, True, 0)
        """
    )
    obj.run(pytorch_code, ["result", "counts", "inverse_indices"])
