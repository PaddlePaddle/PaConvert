...
obj = APIBase("torch.Tensor.byte")


def test_all_type_to_byte():
    dtypes = [
        "torch.float16",
        "torch.float32",
        "torch.float64",
    ...
    # for uint8 --> uint8, since torch.Tensor does not support converting negative
...
def test_complex64_to_byte():
    # for complex --> uint8, we used the new test data
    pytorch_code = textwrap.dedent(
        """
        import torch
