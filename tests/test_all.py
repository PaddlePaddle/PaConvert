...
# RuntimeError: (NotFound) The kernel with key (CPU, NCHW, float32) of kernel `all` is not registered.
def test_case_2():
    pytorch_code = textwrap.dedent(
        '''
        import torch
        a = torch.rand(3, 4).bool()
        result = torch.all(a)
        '''
    )
    obj.run(pytorch_code, ['result'], __file__)

# paddle only support bool, however, pytorch support int/long
def _test_case_3():
    pytorch_code = textwrap.dedent(
        '''
        import torch
