...
def test_case_8():
    pytorch_code = textwrap.dedent(
        """
        import torch
        result = torch.zeros(size=[6, 6], dtype=torch.int64)
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_9():
    pytorch_code = textwrap.dedent(
        """
        import torch
        result = torch.zeros([6, 6], dtype=torch.float64, requires_grad=True)
        """
    )
    obj.run(pytorch_code, ["result"])
...
def test_case_10():
    pytorch_code = textwrap.dedent(
        ...
        import torch
        flag = True
        result = torch.zeros(6, 6, dtype=torch.float64, requires_grad=flag)
        """
    )
    obj.run(pytorch_code, ["result"])
